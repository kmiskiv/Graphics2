...
void a3triangle_render(a3_DemoState const* demoState, a3_Demo_Triangle const* demoMode)
{
	...
	const a3_DemoStateShaderProgram* displayProgram[triangle_pipeline_max][triangle_display_max] =
	{
		{
			0,
		}, {
	...
	const a3_Framebuffer* writeFBO[triangle_pipeline_max] =
	{
		0,
	...
	a3_Demo_Triangle_DisplayProgramName const display = demoMode->display;
	a3_Demo_Triangle_PipelineName const pipeline = demoMode->pipeline;
	a3_Demo_Triangle_TargetName const targetIndex = demoMode->targetIndex[pipeline], targetCount = demoMode->targetCount[pipeline];
...
// hidden volumes
	...
	if (demoState->displayObjectAxes)
	{
		a3_DemoSceneObject const* axesObjects[] = {
			demoState->planeObject, demoState->mainLightObject, demoState->mainCameraObject, demoState->triangleObject,
			0
